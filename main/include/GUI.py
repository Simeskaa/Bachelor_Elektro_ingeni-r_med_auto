...
class MainWindow(QMainWindow):
    def __init__(self, range:float = 2000., delay:int = 5):
        ...
        x = threading.Thread(target=self.item_placement_on_GUI)
        x.start()
...
def test():
    ...
    time.sleep(2)
    window.update_GUI(x=200, y=100, hz= 440, angle_overrule= True)
    logging.info("Thread test is updated 2")
    time.sleep(3)
    logging.info("Thread test is about to update 3")
    window.update_GUI(x=150, y=300, hz= 260, angle_overrule= False)
