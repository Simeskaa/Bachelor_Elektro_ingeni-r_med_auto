...
if __name__ == "__main__":
    ...
    while current_time < time_endpoint:
        ...
        filter_x1 = DC0.mean_value_filter(fake_x1_1)
        filter_x2 = DC1.mean_value_filter(fake_x2_1)
        ...
        for i in range(len(fake_x1_2)):
            holding_list_lp_filter_m1.append(DC0.low_pass_filter(fake_x1_2[i]))
        for i in range(len(fake_x2_2)):
            holding_list_lp_filter_m2.append(DC1.low_pass_filter(fake_x2_2[i]))
        list_lp_filter_m1.extend(holding_list_lp_filter_m1)
        list_lp_filter_m2.extend(holding_list_lp_filter_m2)

        print(f'hps xn {pro.hps(fake_x1_0)}, hps yn BLOKK {pro.hps(filter_x1)}, hps yn LP {(pro.hps(holding_list_lp_filter_m1))}', "\n")
